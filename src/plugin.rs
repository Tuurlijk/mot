...
impl PluginRpcChannel {
    async fn new(mut process: Child) -> eyre::Result<Self> {
        let stdin = process
            .stdin
            .take()
            .ok_or_else(|| eyre!("Failed to take plugin stdin"))?;
        let stdout = process
            .stdout
            .take()
            .ok_or_else(|| eyre!("Failed to take plugin stdout"))?;
        ...
        tokio::spawn(async move {
            ...
            let mut response_map: HashMap<u64, oneshot::Sender<eyre::Result<Value>>> =
                HashMap::new();
    ...
    async fn request<P: Serialize, R: for<'de> Deserialize<'de>>(
        ...
        let params_value =
            serde_json::to_value(params).map_err(|e| eyre!("Failed to serialize params: {}", e))?;
...
impl PluginManager {
    ...
    pub async fn discover_plugins(
        &mut self,
    ) -> eyre::Result<Vec<(String, Result<String, String>)>> {
        ...
        if !self.plugins_dir.exists() {
            ...
            results.push((
                ...
                Ok(t!(
                    "plugin_created_plugins_dir",
                    path = format!("{:?}", self.plugins_dir)
                )
                .to_string()),
            ));
            return Ok(results);
    ...
    async fn load_plugin(&mut self, plugin_dir: &Path) -> eyre::Result<String> {
        ...
        let (enabled, config_read_error) = match fs::read_to_string(&config_path) {
                ...
                match content.parse::<toml::Value>() {
                    ...
                    Err(e) => {
                        ...
                        (
                            true,
                            Some(eyre!(
                                "Failed to parse config.toml {:?}: {}, defaulting to enabled=true",
                                config_path,
                                e
                            )),
                        )
            ...
            Err(_) => {
                ...
                (
                    true,
                    Some(eyre!(
                        "config.toml not found or unreadable in {:?}, defaulting to enabled=true",
                        plugin_dir
                    )),
                )
            }
        ...
        if manifest.plugin.name != dir_name {
            log::warn!(
                ...
                t!(
                    ...
                    manifest_name = manifest.plugin.name,
                    dir_name = dir_name
                )
            );
        }
        ...
        let executable_name = if cfg!(windows) {
            manifest
                ...
                .windows
                .as_ref()
                .unwrap_or(&manifest.executable.default)
        } else {
        ...
        for plugin_name in plugins_to_init {
            if let Some(plugin) = self.plugins.get_mut(&plugin_name) {
                ...
                if !plugin.info.enabled {
                    log::info!(
                        "Skipping initialization for disabled plugin: {}",
                        plugin_name
                    );
                    // Add a specific result indicating it was skipped due to being disabled
                    results.push((
                        plugin_name.clone(),
                        Err(t!("plugin_init_skipped_disabled", name = plugin_name).to_string()),
                    ));
                    continue;
                }
                ...
                if !Path::new(&config_path).exists() {
                    results.push((
                        plugin_name.clone(),
                        Err(t!(
                            "plugin_init_error_config_not_found",
                            name = plugin_name,
                            path = config_path
                        )
                        .to_string()),
                ...
                match fs::read_to_string(&config_path) {
                    Ok(_) => {
                        ...
                        match plugin
                            .channel
                            .request::<Value, bool>(INITIALIZE_METHOD, params)
                            .await
                        {
                            Ok(true) => {
                            ...
                            Err(err) => {
                                    ...
                                    || error_msg.contains("Failed to send request")
                                {
                                    ...
                                    t!(
                                        "plugin_init_communication_error",
                                        name = plugin_name,
                                        error = err.to_string()
                                    )
                                } else {
                                ...
                                results
                                    .push((plugin_name.clone(), Err(detailed_error.to_string())));
                            }
                        }
                    }
                    Err(file_err) => {
                        ...
                        results.push((
                            plugin_name.clone(),
                            Err(t!(
                                "plugin_init_error_config_not_readable",
                                name = plugin_name,
                                path = config_path,
                                error = file_err.to_string()
                            )
                            .to_string()),
                        ));
                    }
                ...
                log::warn!(
                    "Attempted to initialize non-existent plugin: {}",
                    plugin_name
                );
            }
        }
    ...
    pub async fn get_time_entries(
    ...
    ) -> eyre::Result<Vec<PluginTimeEntry>> {
        let plugin = self
            .plugins
            .get_mut(plugin_name)
            .ok_or_else(|| eyre!("Plugin not found: {}", plugin_name))?;
        ...
        match plugin
            .channel
            .request::<Value, Vec<PluginTimeEntry>>(GET_TIME_ENTRIES_METHOD, params)
            .await
        {
            Ok(mut entries) => {
            ...
            Err(err) => Err(eyre!(
                "Failed to get time entries from {}: {}",
                plugin_name,
                err
            )),
        }
    ...
    pub async fn get_all_time_entries(
        &mut self,
        start_date: &DateTime<Utc>,
        end_date: &DateTime<Utc>,
    ) -> eyre::Result<(Vec<PluginTimeEntry>, Vec<(String, String)>)> {
        ...
        for plugin_name in plugin_names {
            match self
                .get_time_entries(&plugin_name, start_date, end_date)
                .await
            {
                Ok(entries) => {
                ...
                Err(err) => {
                    ...
                    if err_string.contains("is disabled")
                        || err_string.contains("is not initialized")
                    {
                        // Log as info, don't add to user-facing errors
                        log::info!(
                            "Skipping entries for plugin '{}': {}",
                            plugin_name,
                            err_string
                        );
                    } else {
            ...
            if let Some(plugin) = self.plugins.remove(&name) {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        ...
                        log::warn!(
                            "Plugin {} shutdown RPC failed: {}. Proceeding with termination.",
                            name,
                            e
                        );
                        errors.push((
                            name.clone(),
                            t!("plugin_shutdown_error", name = name, error = e.to_string())
                                .to_string(),
                        ));
                    }
                }
    ...
    pub async fn debug_plugin_response(
        &mut self,
        plugin_name: &str,
        start_date: &DateTime<Utc>,
        end_date: &DateTime<Utc>,
    ...
    pub async fn debug_plugin_initialization(&mut self, plugin_name: &str) -> eyre::Result<String> {
        ...
        if !dir_exists {
            return Ok(format!(
                "Plugin directory not found for plugin '{}'. Checked: {:?}",
            ...
            #[cfg(unix)]
            {
                ...
                match fs::metadata(&executable_path) {
                    Ok(metadata) => {
                        ...
                        if permissions.mode() & 0o111 == 0 {
                            result = format!(
                                "❌ Executable exists but is not executable: {:?}",
                                executable_path
                            );
                        } else {
                    ...
                    Err(e) => {
                        result = format!(
                            "❌ Failed to check executable permissions: {:?}: {}",
                            executable_path, e
                        );
                    }
                }
        ...
        {
            if let Some(plugin) = self.plugins.get_mut(plugin_name) {
                let config_path = plugin_dir.join("config.toml").to_string_lossy().to_string();
                    ...
                    .await
                {
                    ...
                    Ok(false) => "❌ Plugin returned false during initialization".to_string(),
                    Err(err) => {
